...
## 🎨 Figma Console MCP — SECTION дрейфує, не обгортати наспіх зібране в SECTION

**Заміряно 2026-09-01** при зборі мокапу шапки коментаря (ADR-037) через
`mcp__remote-devices__figma-console__figma_execute`.

Симптом: `figma.createSection()` + `appendChild(...)` дітей-фреймів (самі фрейми
стабільні, `visible:true`, координати не міняються) → сама рамка SECTION
самовільно зʼявляється в новій позиції між викликами (кожного разу інша, не
привʼязана ні до дітей, ні до заданих вручну `section.x/y`) — навіть після
явного переприсвоєння координат під union bounding box дітей одним викликом,
наступний окремий `figma_execute` знову бачив секцію в іншому місці. Користувач
бачить правильну вкладеність у **панелі шарів**, але на **канвасі порожньо**:
рамка секції висить окремо там, де немає реального вмісту.

Причина не встановлена (не воюємо з нею) — **фікс: не використовувати SECTION
як обгортку для дизайну, зібраного через `figma_execute`**. Класти FRAME-и
прямим верхнім рівнем на сторінку (`figma.currentPage.appendChild(...)`),
без проміжного SECTION-контейнера. Якщо SECTION вже створено й «відірвався» —
не намагатись перепризначити йому координати, а забрати дітей назад на
сторінку (`page.appendChild(child)` зберігає absolute-позицію) і видалити
порожню секцію (`section.remove()`).

**Обов'язково перевіряти позицію окремим наступним `figma_execute`-викликом**
(не в тому самому скрипті, де щойно писали) — читання одразу після запису може
показувати інші координати, ніж читання за секунди по тому.

`figma_capture_screenshot`/`figma_take_screenshot` за `nodeId` конкретного
фрейму рендерять вміст коректно завжди — тому скріншот сам по собі не ловить
цю проблему, ловить лише окрема перевірка `node.parent`/`x`/`y`.
