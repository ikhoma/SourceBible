...
**Якщо збираєш DB з нуля через `scripts/build_db.py`:** `verse_map` будується автоматично на кроці після `import_translations` (див. Pipeline нижче).
...
import_macula_greek
import_translations
build_verse_map         ← Strong's overlap alignment; 7 292 non-identity rows
                           Псалми (62 розд.), RST SNG/ZEC/ROM — найбільше зсувів
                           Залежить від: word table + verse table → запускати після обох
import_footnotes
