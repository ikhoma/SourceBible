...
| `ADR-022-analytics-event-collection-strategy.md` | Analytics Event-Collection Strategy | Accepted | Adoption = `feature_adopted_*` once per session (lexicon/commentary/cross_reference/parallel_translation); depth = counters rolled into `study_session_summary` (`*_views_count`, `lexicon_nav_count`, etc.) — NOT a discrete event per tap, to stay under free-tier (1M/mo); search/annotations/translation_switched kept discrete (low-volume, high-value); `SessionTracker` owns once-per-session adoption + counters; loses per-view timestamps + commentary author/strongs_id (acceptable). Supersedes the per-tap Slice 3 draft |
