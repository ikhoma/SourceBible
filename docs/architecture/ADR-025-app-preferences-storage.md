...
> ⚠️ **Скасовано 2026-08-03 — див. Amendment нижче.** Дефолт тепер `lastUsed`.
...
## Amendment 2026-08-03 — дефолт `lastUsed` + стартовий переклад під мову

### 1. Дефолт `translationLaunchBehavior`: `fixed` → `lastUsed`

Попередній амендмент обрав `fixed`, щоб оновлення не змінило мовчки поведінку
для наявних користувачів. **Цей аргумент відпав:** на момент зміни база
користувачів — тестери, тож зберігати сумісність із дорелізною поведінкою немає
для кого. Ціна аргументу була реальна: дефолт вимагав, щоб людина сама знайшла
перемикач у Меню заради поведінки, якої вона й так очікує.

`lastUsed` = застосунок відкривається тим перекладом, яким читали востаннє.
`fixed` лишається явним вибором у Меню — для тих, хто хоче завжди стартувати з
одного перекладу незалежно від того, куди заглядав.

⛔ Якщо колись зʼявиться помітна база **до** цієї зміни, повторний фліп дефолту
вже потребуватиме одноразової міграції за прапорцем, а не правки константи.

### 2. Стартовий переклад під мову інтерфейсу

Ланцюг фолбеків у `ReadingPositionStore.launchTranslationId()`:

| крок | джерело | коли спрацьовує |
|---|---|---|
| 1 | `lastUsedTranslationId` | режим `lastUsed` і вже щось читали |
| 2 | `defaultTranslationId` | є явний вибір у Налаштуваннях (б'є будь-який дефолт) |
| 3 | `AppLanguage.defaultTranslationId` | **найперший запуск** — UK → `UBIO`, інакше `KJV` |

Крок 3 — новий. Раніше ланцюг обривався на кроці 2 і повертав `nil`, а
`ReaderViewModel` падав на компільований `Translation.defaultTranslation` (KJV).
Тобто **україномовний користувач на першому екрані бачив KJV** — після того, як
сам обрав українську мову інтерфейсу.

`AppLanguage.resolved` / `.defaultTranslationId` живуть біля `AppLanguage`
(`SourceBibleApp.swift`), а не в сторі: мову треба знати ще до побудови сцени
(свізл бандла локалізації), і логіка «збережене → системне → звузити до
підтримуваних» тепер одна на всіх. `SourceBibleApp.defaultLanguage` став
однорядковим делегатом до неї замість копії.

Невідомий id тут безпечний: `ReaderViewModel` звіряє результат із перекладами,
які реально є в базі, і мовчки падає на `Translation.defaultTranslation`.

**Перевірено на симуляторі** (launch-args, чисті ключі): `appLanguage=uk` → UBIO,
`appLanguage=en` → KJV.
