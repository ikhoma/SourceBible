...
## Amendment 2026-08-17 — пейджинг перетинає межу книги (ux-020)

**Status:** Accepted — реалізовано 2026-08-17.

### Контекст

Двоє тестерів: дочитавши останню главу книги, треба йти в навігацію і шукати наступну —
багато кліків там, де очікується один жест. Первинне рішення свідомо обмежило область
пейджера книгою: `hosting(for:)` віддавав `nil` за межами `1…chapterCount`, і саме це
«чисто зупиняло» пейджер. Для читання поспіль ця межа штучна: Малахія 4 і Матвія 1 —
сусідні сторінки Писання, а не два різні простори.

### Рішення

**Одиниця сторінки більше не «номер глави в книзі», а `ChapterRef` (книга + глава),
адресований наскрізним порядковим номером глави в каноні.** Послідовність будується з
`allBooks` (`ORDER BY num`) префіксними сумами `chapterCount`; `globalChapterIndex(bookId:chapter:)`
і `chapterRef(atGlobalIndex:)` — O(1) в обидва боки.

- **Безшовно** (рішення Івана 2026-08-17): свайп і чеврон однаково переходять у сусідню
  книгу без проміжного екрана. Афорданс уже є і нічого не коштує — глава 1 показує
  **обкладинку книги** (ADR-017), тобто перехід межі видно очима.
- **Кінці канону — стоп:** Бут. 1 назад і Об. 22 вперед лишаються глухими (чеврон
  неактивний, свайп упирається). Кільце Об.→Бут. відкинуте: несподіванка при випадковому
  свайпі коштує дорожче за зручність річного плану.
- **Порядок книг** — канонічний `book.num`, не `book_name.sort_order` (ADR-018): усі п'ять
  наявних перекладів протестантського канону йдуть однаково, а `sort_order` живить пошук.
  ⛔ Якщо колись з'явиться переклад з іншим порядком книг — джерелом послідовності мусить
  стати `sort_order`, інакше пейджер поведе не туди.

### Що це змінює в коді

- `ChapterScrollContent` приймає `ChapterRef`, а не `chapter: Int` — інакше сусідня
  сторінка рендериться з `vm.currentBook` і показує чужі вірші. `versesForPage(_:)` і
  `chapterHeading(for:)` теж беруть `ChapterRef`; ключ кешу сторінок став
  `book|chapter|translation`.
- `ChapterPagerView`: `view.tag` = наскрізний індекс; коміт свайпу ставить і книгу, і главу
  (`goToChapter(globalIndex:)`), зберігаючи відкладений на такт `loadChapter()`.
- Чеврони: замість `currentChapter <= 1` / `>= chapterCount` — `canGoPrevChapter` /
  `canGoNextChapter` (межі канону). Ті самі методи живлять iOS 18 (`EdgeSwipeNavigator`),
  тож перехід між книгами працює на обох шляхах без окремої гілки.

### Що НЕ змінилось (інваріанти ADR-026 і сусідів)

Сам вигляд глави (`ChapterScrollContent` — правило «контейнер міняється, вигляд ні»),
скидання скролу на верх сторінки (`willTransitionTo`), блокування пейджингу в Study Mode
(N1), ADR-021 пінування, ADR-017 обкладинки, ADR-014 `.id()`, відкладений на runloop-такт
`loadChapter()` після коміту свайпу, `pendingRestoreAnchorId` і збереження позиції читання.

### Знайдено на ревʼю перед мерджем (2026-08-17) — виправлено

Дві дірки цього амендменту, обидві від того, що **індекс 0 — валідна сторінка**, тоді як
номер глави 0 не існував:

1. **Холодний старт.** `allBooks` вантажиться асинхронно після init, а рідер малюється
   раніше. Поки список порожній, послідовність не могла описати жодної сторінки:
   `hosting(for:)` віддавав nil, `setViewControllers` не викликався — **порожній рідер**
   із двома вимкненими чевронами до першого стороннього оновлення стану. Раніше цього
   не могло статись: межі бралися з `currentBook.chapterCount`, а `currentBook` має
   дефолт. Фікс — `sequenceBooks`: поки поточної книги немає в `allBooks`, послідовність
   = сама поточна книга, тобто рівно до-ux-020 поведінка.
2. **`?? 0` у `currentGlobalChapterIndex`.** `currentBook` і `currentChapter` — дві окремі
   `@Published`, тож коміт, що змінює обидві, на мить спостерігається як (нова книга,
   стара глава) — Матвій/150 при виході з Пс. 150. `globalChapterIndex` для такої пари
   віддає nil, а фолбек у 0 — це **Буття 1**: пейджер тихо переїхав би на початок Біблії,
   і наступний свайп зберіг би це як позицію читання. Фолбек тепер клампиться в початок
   ПОТОЧНОЇ книги.

Урок, який варто пам'ятати при будь-якій зміні адресації сторінок: коли простір індексів
0-базований, «не знаю» не можна кодувати нулем — нуль тут означає конкретну сторінку.

Не виправлялось (успадковане від фази 2, не регресія цього амендменту, кандидати в окремі
баги): `isTransitioning` скидається лише в completion-блоці `setViewControllers` — якщо
той не прийде, програмна навігація замовкне; програмний перехід під час свайпу мовчки
перетирається комітом свайпу; `loadChapter()` чистить весь кеш сторінок, включно з уже
змонтованими сусідами.

### Наслідки

- Плюс: читання поспіль без навігації; «наступна книга» коштує той самий жест, що
  «наступна глава».
- Мінус: випадковий свайп на межі книги тепер веде в іншу книгу. Прийнято свідомо —
  повернення коштує один свайп назад, а обкладинка робить перехід очевидним.
- Префетч сусідів на межі підвантажує першу главу наступної книги — той самий один
  `loadChapter` за сторінку, окремої ціни немає.
