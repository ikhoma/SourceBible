...
**Status:** Accepted (реалізовано 2026-09-11 — Прогін 2 + Swift кроки 5-9; крок 10 «візуальне QA» відкритий) — Варіант B, з перезбіркою бази
**Date:** 2026-09-05
**Deciders:** Іван
...
**Amendment 2026-09-11 (реалізація):** кроки 5-9 «Обсягу реалізації» виконано
на гілці `work` (некомічено — комітить Іван), Прогін 2 прогнано і звірено
проти живої `sourcebible.db` (не лише проти сирих TSV, як на етапі
проєктування): усі 14 нових колонок присутні й заповнені (гебр. дієслова
99.80%, грец. дієслова 100%), набори значень усіх полів співпали з
попереднім TSV-аудитом один в один, жодного витоку сентинелів `unknown:*`.
Побічно знайдено й закрито прогалину в термінологічному словнику з
2026-09-05: породи `hithpalpel` не було серед 34 нових ключів `MorphKey`
(13 форм у корпусі) — додано ключ і переклад. Xcode build succeeded, 0
warnings/errors. Крок 10 (візуальне QA на симуляторі/пристрої) лишається
відкритим — обмеження агентської сесії, керувати симулятором наживо не
вдається; дані звірені напряму із БД, але візуально не переглянуто.
...
| `mood` | грецька | `indicative` / `participle` / `infinitive` | Greek `mood` (UI розводить на «Спосіб» і «Неособову форму» — див. рецензію Opus вище; назва рядка уточнена рішенням 2026-09-11, було «Форма», див. нижче) |
...
| `mood` | Спосіб | ТІЛЬКИ 4 справжні способи: дійсний/наказовий/умовний/бажальний. `participle`/`infinitive` — окремий рядок **«Неособова форма»** (ключ `morph.row.non_finite_form`), не «Спосіб» (виправлено рецензією Opus, див. вище; назва рядка «Форма»→«Неособова форма» — рішення 2026-09-11, п.7 нижче: збігалась із заголовком секції «Форма у [вірш]») |
| `degree` | **Ступінь порівняння** | звичайний ступінь рядком НЕ показується (дефолт, не інформація); вищий/найвищий — показуються |
| `morph_type` (гебр., було `verb_type`) | **Тип** (не «Форма» — рішення 2026-09-11: та сама причина, збіг із «Форма у [вірш]»; Іван дав вибір «форма чи тип» 2026-09-05, обрано «Тип») | для дієслів: перфект/імперфект (звична термінологія застосунку — існуючий рядок/ключ `rowAspect`, лише перейменована мітка й нове джерело значення) + wayyiqtol НЕ згортається в імперфект (14 978 вживань, окремий ключ `aspectWayyiqtol` уже існує — зберегти). Не-дієслівні значення (`common`/`proper`/`pronominal`/…) імпортуються, у UI v1 не показуються |
...
| випадок | що зробити |
|---|---|
| Дієприкметник (participle, 6 653) і інфінітив (2 285) — грецькі, лежать у полі `mood`, НЕ в «Спосіб» | **Виправлено рецензією Opus:** окремий рядок «Неособова форма» (не «Спосіб»); для participle TSV дає відмінок/рід/число прямо в `case`/`gender`/`number` — спецлогіка потрібна лише для розведення `mood`→{Спосіб, Неособова форма} |
...
- Дієприкметник (напр. будь-який `V-PAP-*`) → Відмінок/Рід/Число замість
  Особи, без спецкоду.
- **Підтверджено 2026-09-11 напряму з `sourcebible.db`** (не з TSV): Рим 6:14
  κυριεύσει → `tense=future, voice=active, mood=indicative, person=third,
  number=singular`; Пс 1:1 слот 6 (בַּעֲצַת, складене слово) → голова слота
  `עֲצַת` віддає `state=construct, pos=noun` коректно — точно той самий SQL,
  що виконує `DatabaseService.loadWords()`.
- **Досі потрібне візуальне QA на пристрої/симуляторі** (той самий
  застережний рядок, що й в ADR-033, — керувати симулятором наживо із сесії
  агента не вдається; збірка зелена й дані звірені напряму із БД, але
...
4. **Корпусний диференціал (найважливіший запобіжник):** згенерувати таблицю
   ...
   Ос. 4:6) її не бачить.
5. ✅ **Зроблено 2026-09-11.** `MorphologyDecoder`: прибрати `hebrewStem`/`hebrewAspect`/`personLabel`/
...
6. ✅ **Зроблено 2026-09-11** (66 нових ключів + `stemHithpalpel`, знайдений
   як прогалину під час реалізації — див. amendment вище). Нові ключі
   `MorphKey` (Відмінок/Час/Стан/Спосіб/Ступінь порівняння +
   ...
   Рядок для participle/infinitive (відокремлено від «Спосіб») отримує ключ
   `morph.row.non_finite_form` / підпис **«Неособова форма»** — НЕ «Форма»,
   як планувалось спочатку (рішення 2026-09-11, див. п.7 нижче: «Форма» вже
   зайнято заголовком секції `sectionFormInContext` — «Форма у [вірш]» — і
   збіг двох «Форма» на одному екрані плутав би).
7. ✅ **Зроблено 2026-09-11.** `WordTabContent` / `WordMeaningView` (рішення 2026-09-11):
   - **`rowComposition` («Склад слова») переїжджає** з `morphologyRows()`
     (секція «Морфологія») у `contextSection()` (секція «Форма у [вірш]»,
     `MorphKey.sectionFormInContext`) — рядком після «Слово»/«Транслітерація».
     Причина: композиція («прийм. + ім.») описує КОНКРЕТНИЙ запис слова в
     ЦЬОМУ вірші (Macula-слот `!N` — угруповання per-occurrence, та сама лема
     деінде може стояти без приліпленого прийменника/артикля), а не
     граматичну категорію леми — тобто це факт про форму, а не про
     морфологію голови. Секція «Морфологія» лишається чистою: лише
     граматичні категорії голови слота (частина мови, відмінок/час/стан/
     спосіб/… , синтаксична роль).
   - Нові рядки в секції «Морфологія» з нових колонок (`ADR-040`), включно з
     окремим рядком «Неособова форма» (ключ `morph.row.non_finite_form`,
     див. п.6) для participle/infinitive, відокремленим від «Спосіб».
8. ✅ **Зроблено 2026-09-11** (див. ADR-033 amendment). **ADR-033:** перевірити `canonicalHebrewStem()` після розширення до 34
...
9. ✅ **Зроблено 2026-09-11** (bug-053 → closed). Закрити bug-053: не видаленням `greekPartOfSpeech()`, а заміною
