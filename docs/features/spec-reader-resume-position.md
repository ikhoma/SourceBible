...
**Оновлено 2026-08-02: режим і значення обʼєднані в ОДИН рядок.**

- Один рядок «Переклад при відкритті» → `TranslationLaunchPickerView`.
  Значення в рядку — або «Останній відкритий», або id перекладу.
- У списку дві секції: «Останній відкритий» (із поясненням), далі всі переклади
  з назвою й мовою. Вибір перекладу пише ОБИДВА ключі — режим `.fixed` і `defaultTranslationId`.
- Спершу було дві настройки, і рядок значення доводилось ховати в режимі `lastUsed`,
  бо він переставав чимось керувати. Тепер «Останній відкритий» — просто ще один
  пункт того самого списку, і ховати нічого не треба.
- ⛔ `DefaultTranslationPickerView` лишається окремим і БЕЗ «Останнього відкритого»:
  його перевикористовує фільтр перекладу в пошуку, де цей пункт не має сенсу.
