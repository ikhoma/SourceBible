...
**Форма контролів на iOS 18 — 2026-08-03.** Дві зміни в один бік: узгодити
застосунок сам із собою, а не з двома різними трактуваннями.

- **Кнопки й сегментовані контроли — капсульні.** `.bordered*` на 18 дає
  прямокутник із малим радіусом, поруч із рештою читається як з іншої епохи.
  Форма міняється системним `.buttonBorderShape(.capsule)`, не ручним
  `cornerRadius`. Сегментований трек — `clipShape(Capsule())`, бо SwiftUI не дає
  його форму; внутрішня пігулка виділення лишається зі своїм радіусом.
  Обидва хелпери (`legacyCapsuleButton`, `legacySegmentedCapsule`) — **no-op на
  iOS 26**, там форма вже капсульна.
- **Чипи фільтрів у Пошуку — без заливки.** `LegacyControlCapsule`
  (`secondarySystemFill`) видалено. 2026-08-01 капсулу додали з аргументом «смуга
  фільтрів — не нав-бар, тут вона доречна», тимчасом як тулбар рідера свідомо
  лишили голим. Наслідок: один і той самий контрол — «пікер із чевроном» — мав
  два різні вигляди на сусідніх екранах. Консистентність усередині застосунку
  переважила різницю контекстів. Падінги лишились: вони тримають зону дотику.

**Висота шапки Пошуку = нав-бар рідера, 2026-08-03.** Три ітерації, бо я двічі
міряв не те. Записую, що саме, щоб не повторювати.

| спроба | що зробив | результат |
|---|---|---|
| 1 | падінги 11·2 + «`.headline` ≈ 22» | ~50 pt — арифметика по номінальному line height, а не по фактичній висоті `HStack` із текстом і символом |
| 2 | `.frame(height: UINavigationBar().sizeThatFits().height)` | 44 pt, а нав-бар усе одно вищий на екрані |
| 3 | + відʼємна поправка на Dynamic Island | збіг |

**Справжня причина — не висота, а точка відліку.** На пристроях з Dynamic Island
`statusBarFrame.height` = 54, а `safeAreaInsets.top` = 59: safe area піднята, щоб
обійти острівець, а `UINavigationBar` кладеться одразу під статус-бар.
`safeAreaInset(edge: .top)` починає вміст із 59 — звідси стабільні ~5 pt, скільки
не підбирай висоту. Компенсовано відʼємним верхнім падінгом.

**Еталон — заміряний бар, не номінальний.** `readerVM.toolbarBottomY`
(`NavBarBottomReader`) дає 97.7, тимчасом як `UINavigationBar().sizeThatFits()`
обіцяє 44.0 → 98.0. Ті 0.3 і були останнім хвостом. Номінальна висота лишилась
лише фолбеком на випадок, коли рідер ще не рендерився.

**Заміряно:** рідер `navBottom = 97.7`, шапка Пошуку `maxY = 97.7`, висота 38.7.

⚠️ Спосіб заміру: тимчасові launch-арги `-startTab` / `-startQuery` (щоб потрапити
на сторінку результатів без тапів) + червоні `Text` із координатами. Усе прибрано
після заміру — у коді цього немає. Якщо доведеться міряти знову, це найшвидший шлях.
