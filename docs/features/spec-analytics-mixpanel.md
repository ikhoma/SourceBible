...
| `study_session_summary` | `duration_s`, `verses_read`, `study_tool_opens`, `annotations_created`, `searches` | кінець сесії (background, grace ~30с) |
...
| Event | Props | Коли |
|---|---|---|
| `original_opened` | `book_id` | відкрито Original/Strong's на слові |
...
| `verse_card_added` | — | картка вірша (нові Нотатки) |
| `word_card_added` | — | картка слова (нові Нотатки) |
| `folder_created` / `tag_added` | — | організація нотаток |
...
MSS = study_session_summary WHERE
      duration_s >= 120
  AND verses_read >= 2
  AND (study_tool_opens >= 1 OR annotations_created >= 1)
```

- Поріг **тюнабельний у Mixpanel** без релізу.
- `study_tool_opens` інкрементується додатком на кожен `original_opened` / `strongs_viewed` / `commentary_opened` / `crossref_opened` / `word_usage_opened` у межах сесії.
...
- Події нових Нотаток (`verse_card_added` тощо) залежать від фічі Нотаток — інструментувати разом із нею.

## Implementation Slices (для черговості; перший зріз — кандидат на Fable-тест у worktree)

1. **Каркас:** `AnalyticsService` + `MixpanelAnalytics` + `NoopAnalytics`, гейтинг, distinct_id, `PrivacyInfo.xcprivacy`, token через xcconfig. (Acceptance: події летять у бета, no-op у release.)
2. **Engagement + Search:** `app_opened`, `study_session_summary`, `search_committed`, `search_result_opened`.
3. **Feature adoption:** події по існуючих фічах (Reader/Original/Strong's/commentary/crossref/usage/translation/анотації).
4. **Нові Нотатки:** card/folder/tag події — разом із фічею Нотаток.
