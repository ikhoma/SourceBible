...
**Стрибок картки при першій появі (amend 2026-08-03).** Картка презентувалась,
змінювала висоту, вміст переїжджав удруге. Виявилось **три незалежні шляхи** до
одного симптома — закриті всі, бо кожен лишався б міною.

| # | шлях | фікс |
|---|---|---|
| 1 | шіт показувався ДО того, як проба заміряла висоту | `presentIfReady()` — показ лише коли зійшлись «згода потрібна» І «висота заміряна» |
| 2 | проба перемірювала ПІСЛЯ показу (перекладка при завантаженні рідера, осідання скляних кнопок на iOS 26, Dynamic Type) і міняла детент відкритого шіта | апдейти проби ігноруються, щойно `showSheet == true` |
| 3 | показ стартував ВСЕРЕДИНІ того ж проходу розкладки | `Task { @MainActor in showSheet = true }` |

**№3 і був тим, що бачив користувач.** Замір показував ідеальні числа
(`p388 SHOW@388`, `inner: 388` — детент не змінюється, картка рівно по ньому), а
на екрані стрибало: трейс фіксує НАМІР, а UIKit малює те, що встиг прочитати.
`presentIfReady()` викликається з `onGeometryChange`, тобто під час розкладки;
виставити `showSheet` там — почати презентацію в тому ж проході, у якому щойно
змінився `sheetHeight`. Перший кадр виїжджає зі старою висотою й підганяється.

⛔ Урок на майбутнє: якщо числа сходяться, а на екрані ні — дивитись не на
значення, а на **момент**, коли стан змінюється відносно циклу розкладки.

№1 і №2 самі по собі симптом не пояснювали, але лишати їх не можна: обидва дають
той самий стрибок за інших умов (холодний старт пристрою, зміна Dynamic Type).
