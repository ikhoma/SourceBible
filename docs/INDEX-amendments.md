...
**Status:** **Accepted** (2026-08-03; amended 2026-09-11)

**Amendment 2026-09-11:** механізм звірки переписано в межах ADR-040.
`canonicalHebrewStem()` більше не парсить символ стисненого коду — бере
`word.stem` напряму з нової колонки `word` (заповненої з Macula TSV при
перезбірці). Покриття зросло з 8 до **34** порід/арам. еквівалентів, і
складене гебрайське слово (`C·Vqp3ms`) більше не повертає мовчки `nil` —
колонка завжди належить голові слота, а не проклітиці. Досі не перевірено
візуально на пристрої/симуляторі. Повний запис — amendment у самому ADR-033.
